...
const (
	...
	NextWaitDuration = time.Second * 2
)
...
func (u *Unit) loop() {
	...
	for {
		...
		u.wg.Add(1)
		resp, err := getURL(u.url, time.Second*20)
		if err != nil {
		...
		u.isErrorStatus = 0
		u.parsePullResponse(resp)
		resp.Body.Close()
		log.Debug("req-url-ok", "key", u.key)
		u.wg.Done()
	}
}

func (u *Unit) parsePullResponse(resp *http.Response) {
	if resp.Header.Get("Data-Type") == "pack" {
		u.parseResponsePacket(resp)
		return
	}
	u.parseResponseMetrics(resp)
}

func (u *Unit) parseResponseMetrics(resp *http.Response) {
	...
	if metricsChan != nil && len(metrics) > 0 {
		metricsChan <- metrics
	}
}

func (u *Unit) parseResponsePacket(resp *http.Response) {
	dataLen, _ := strconv.Atoi(resp.Header.Get("Data-Length"))
